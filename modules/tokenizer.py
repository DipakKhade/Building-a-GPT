from typing import List

# custom simple tokenzier  #dipak

class CustomTokenizer:
    trainig_data = ''
    unique_chars = ''

    encoding_mapper: dict[str: int] = {}
    decoding_mapper: dict[int: str] = {}

    def __init__(self, trainig_data_file_path):
        with open(trainig_data_file_path, 'r+') as f:
            content = f.read()
            CustomTokenizer.trainig_data: str = content
            CustomTokenizer.unique_chars: str = CustomTokenizer.get_all_unique_chars_from_traning_txtdata()
            CustomTokenizer.set_encoding_decoding_mapping()

    @staticmethod
    def get_all_unique_chars_from_traning_txtdata() -> str:
        chars = ''
        x = list(set(CustomTokenizer.trainig_data))
        x.sort()
        for char in x:
            chars+= char
        return chars
    
    @staticmethod
    def set_encoding_decoding_mapping() -> None :
        CustomTokenizer.encoding_mapper = [ {char: index} for index, char in enumerate(CustomTokenizer.unique_chars) ]
        CustomTokenizer.decoding_mapper = [ {index: char} for index, char in enumerate(CustomTokenizer.unique_chars) ]
        return

    @staticmethod
    def encoder(input_txt: str) -> List[int]:
        encodings = []
        for char in input_txt:
            for encoded_char in CustomTokenizer.encoding_mapper:
                if char == list(encoded_char.keys())[0]:
                    encodings.append(encoded_char[char])
        return encodings

    @staticmethod
    def decoder(encodings: list[int]):
        decoding = []
        for encode in encodings:
            for decode_int in CustomTokenizer.decoding_mapper:
                if encode == list(decode_int.keys())[0]:
                    decoding.append(decode_int[encode])
        return ''.join(decoding)

# encoded_txt = CustomTokenizer('traning_input_txt.txt').encoder('my name is dipak.')
# decoded_txt = CustomTokenizer('traning_input_txt.txt').decoder(encoded_txt) 

# print(encoded_txt, decoded_txt)




#tiktoken -- Byte Pair Encoding (BPE) Tokenizer is the thing that ChatGPT uses for tokenzing the input txt
import tiktoken

class BPETokenizer:
    def __init__(self):
        self.tokenzier = tiktoken.encoding_for_model("gpt-4o")

    def encoder(self, input_txt: str) -> List[int]:
        return self.tokenzier.encode(input_txt)
    
    def decoder(self, tokens: List[int]) -> str:
        return self.tokenzier.decode(tokens)

# bpe = BPETokenzier()
# bpe_encoded_txt = bpe.encoder('hello from dipak')
# bpe_decoded_txt = bpe.decoder(bpe_encoded_txt)

# print(bpe_encoded_txt, bpe_decoded_txt)



